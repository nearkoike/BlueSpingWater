			...
			<div class="col-md-12">
                ...
                @foreach ($sales as $item)
                  ...
                  @foreach ($item->carts as $cart)
                    <td><img src="{{ $cart->product->image }}" height="50px" alt=""></td>
										<td>{{ $cart->product->name }}</td>
										<td>{{ $cart->product->price }}</td>
										<td><input type="text" name="quantity" id="quantity" value="{{ $cart->quantity }}"></td>
										<td>{{ $cart->price }}</td>
									</tr>
                  @endforeach
                  <h3>Sale ID: {{ $item->id }} | Total: {{ $item->amount }}</h3>
                  
                    <a href="/admin/delete/{{ $item->id }}" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i></a>
								</tbody>
                  <hr>
							</table>
                  @endforeach
